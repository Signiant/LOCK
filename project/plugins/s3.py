...
def write_s3_file(config_map, username, **key_args):
    ...
    file_contents = "%s\n%s\n" % (
        key_args.get("access_key").replace(
            "<new_key_name>", values.access_keys[username][0]
        ),
        key_args.get("secret_key").replace(
    ...
    if values.DryRun is True:
        logging.info(
            f"User {username}: Dry run, upload file %s to s3 bucket %s"
            % (file_path, bucket)
        )
        result = True
    else:
        try:
            logging.info(
                f"User {username}: Attempting to upload to s3 bucket %s at path %s"
                % (bucket, file_path)
            )
            response = client.put_object(
                Bucket=bucket, Body=file_contents.encode(), Key=file_path
            )
