...
def __get_bitbucket_token(config_map, username, **kwargs):
    ...
    try:
        ...
        bb_api_key = ssm_client.get_parameter(
            Name="LOCK.bb_api_key", WithDecryption=True
        )["Parameter"]["Value"]
        bb_api_secret = ssm_client.get_parameter(
            Name="LOCK.bb_api_secret", WithDecryption=True
        )["Parameter"]["Value"]
    except Exception as e:
        logging.error(
            f"User {username}: Error retrieving Bitbucket credentials from Parameter Store: {e}"
        )
        return None
...
def __put_variable(username, api_token, workspace, variable_uuid, variable_details):
    ...
    if response.status_code != 200:
        logging.error(
            f"User {username}: Error updating Bitbucket variable {response.status_code}: {response.text}"
        )
...
def update_variables(config_map, username, **kwargs):
    ...
    if not api_token:
        logging.error(
            f"User {username}: Missing Bitbucket API token. Unable to update variables for {username}: aborting"
        )
        return None
    ...
    if values.DryRun:
        logging.info(
            f"User {username}: Dry run. No Bitbucket workspace variables will be updated."
        )
        return None

    logging.info(f"Updating access key variable for {username}")
    __update_variable(
        username, api_token, workspace, access_key_uuid, aws_access_key_id
    )

    logging.info(f"Updating secret access key variable for {username}")
    __update_variable(
        username, api_token, workspace, secret_access_key_uuid, aws_secret_access_key
    )
