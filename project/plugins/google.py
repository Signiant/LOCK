...
def set_secret_manager(configMap, username, **key_args):
    ...
    if values.DryRun is True:
        ...
        try:
            parent_access = client.secret_path(
                key_args.get("project_id"), key_args.get("key_name")
            )
            ...
            logging.error(
                f"User {username}: Exception trying to update Google Access Key - Key may need to be updated manually. Exception: {e}"
            )
            return
        try:
            parent_secret = client.secret_path(
                key_args.get("project_id"), key_args.get("key_secret")
            )
            ...
            logging.error(
                f"User {username}: Exception trying to update Google Secret Key - Key may need to be updated manually. Exception: {e}"
            )
            return

        logging.info(
            f"User {username}: Access key and Secret key written to Secret Manager"
        )
        pass
...
def rotate_gcp_instance_group(username, auth, regions, max_unavailable):
    ...
    try:
        ...
        logging.error(
            f"User {username}: Exception trying to authenticate with google: {e}"
        )
        return
    ...
    for item in regions:
        ...
        try:
            ...
            return
        logging.debug(
            f"User {username}: Project: {project} Instance_group {instance_group} Region {region}"
        )
        ...
        try:
            ...
            instance_template = project_list["items"][0]["versions"][0][
                "instanceTemplate"
            ]
        except Exception as e:
            logging.error(
                f"User {username}: Unable to retrieve current instance template: {e}"
            )
            return
        logging.debug(
            f"User {username}: template: {project_list['items'][0]['versions'][0]['instanceTemplate']}"
        )
        ...
        if values.DryRun:
            logging.info(
                f"User {username}: Dry run. Instance group {instance_group} will not be rotated."
            )
            return
    ...
    if values.DryRun is True:
        ...
        try:
            ...
            storage_client = storage.Client(
                key_args.get("project_id"), credentials=credentials
            )

        except Exception as e:
            logging.error(
                f"User {username}: Exception trying to update flight-gateway cred - Key may need to be updated manually. Exception: {e}"
            )
            return
        ...
        aws_cred = (
            "[default]\naws_access_key_id = {0}\naws_secret_access_key = {1}\n".format(
                values.access_keys[username][0], values.access_keys[username][1]
            )
        )

        # encrypt that file through kms
        try:
            aws_cred_encrypted = encrypt_symmetric(
                kms_client,
                key_args.get("project_id"),
        ...
        try:
            ...
            upload_blob(
                username,
                storage_client,
                key_args.get("bucket_name"),
                encrypted_file_path,
                key_args.get("file_name"),
            )
        except Exception as e:
            logging.error(
                f"User {username}: Exception trying to upload encrypted cred to gcp bucket. Exception: {e}"
            )
            return
...
def encrypt_symmetric(
    kms_client, project_id, location_id, key_ring_id, key_id, plaintext
):
    ...
    request = kms_v1.EncryptRequest(
        name=key_name, plaintext=plaintext_bytes, plaintext_crc32c=plaintext_crc32c
    )
...
def upload_blob(
    username, storage_client, bucket_name, source_file_name, destination_blob_name
):
    ...
    logging.info(
        f"User {username}: File {source_file_name} uploaded to {destination_blob_name}."
    )
