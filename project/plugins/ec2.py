...
def get_iam_e_session(**key_args):
    return boto3.Session(
        profile_name=values.profile, region_name=key_args.get("region")
    )


def get_elb_client(config_map, **key_args):
    if values.profile is not None:
        session = get_iam_e_session(**key_args)
    ...
    for instance in response.get("Reservations"):
        if instance.get("Instances")[0].get("State").get("Name") == "running":
            ...
            try:
                ...
                logging.warning(f"User {username}: {e}")
    logging.debug(
        f"User {username}: Found the following instances with Name %s: %s"
        % (instance_name, str(instance_ids))
    )
    return instance_ids
...
def get_describe_instance_status(client, instance_id):
    ...
    return (
        response.get("InstanceStatuses")[0]
        .get("InstanceStatus")
        .get("Details")[0]
        .get("Status")
    )
...
def terminate_instances(config_map, username, **key_args):

    elb_client = get_elb_client(config_map, **key_args)
    ec2_client = get_ec2_client(config_map, **key_args)
    ...
    for instance_name in instance_names:
        ...
        for instance_id in instances:
            ...
            if values.DryRun is True:
                logging.info(
                    f"User {username}: Dry run instance: %s %s"
                    % (instance_name, instance_id)
                )
            else:
                # Remove instance from loadbalancer
                logging.info(
                    f"User {username}: Removing %s from Targetgroup" % instance_id
                )
                if not _remove_instance_from_targetgroup(
                    elb_client, tg_arn, instance_id
                ):
                    logging.warning(
                        f"User {username}: Failed to remove instance from target group"
                    )
                else:
                    logging.info(
                        f"User {username}: Successfully removed %s from Targetgroup - pausing for 60 seconds"
                        % instance_id
                    )
                ...
                while not reachable:
                    checkinstancelist = list_instances(
                        username, ec2_client, instance_name
                    )
                    if len(checkinstancelist) > 0:
                        ...
                        for inst in new_instance_ids:
                            if inst not in new_instances and inst not in instances:
                                ...
                                logging.info(
                                    f"User {username}: Found newly launched instance: %s"
                                    % inst
                                )

                        for new_inst in new_instances:
                            instance_reachability = get_describe_instance_status(
                                ec2_client, new_inst
                            )
                            if instance_reachability == "passed":
                                reachable = True
                                # check load balancer
                                if not _is_instance_in_service(
                                    elb_client, tg_arn, new_inst
                                ):
                                    logging.info(
                                        f"User {username}: Waiting for instance %s to be InService..."
                                        % new_inst
                                    )
                                    time.sleep(45)
                                    while not _is_instance_in_service(
                                        elb_client, tg_arn, new_inst
                                    ):
                                        logging.info(
                                            f"User {username}: Waiting for instance %s to be InService..."
                                    ...
                                    logging.info(
                                        f"User {username}: New instance %s is InService - move on"
                                        % new_inst
                                    )
                    if not reachable:
                        if len(new_instances) < 1:
                            logging.info(
                                f"User {username}: Waiting for newly launched instance(s)..."
                            )
                        else:
                            logging.info(
                                f"User {username}: Waiting for newly launched instance to pass status checks..."
                            )
                        time.sleep(45)


def get_loadbalancername(_, instance_name, client, **__):
    response = client.describe_load_balancers()

    list_loadbalancers = response.get("LoadBalancerDescriptions")
    for loadbalancer in list_loadbalancers:
        balancer_tags = client.describe_tags(
            LoadBalancerNames=[loadbalancer.get("LoadBalancerName")]
        )
...
def get_target_group_arn(client, instance_name):
    ...
    for loadbalancer in list_loadbalancers:
        list_tgs = client.describe_target_groups(
            LoadBalancerArn=loadbalancer.get("LoadBalancerArn")
        )
        for targetgroup in list_tgs.get("TargetGroups"):
            targetgroup_tags = client.describe_tags(
                ResourceArns=[targetgroup.get("TargetGroupArn")]
            )
...
def _is_instance_in_service(elb_client, tg_arn, instance_id):

    response = elb_client.describe_target_health(
        TargetGroupArn=tg_arn, Targets=[{"Id": instance_id}]
    )
    return (
        response.get("TargetHealthDescriptions")[0].get("TargetHealth").get("State")
        == "healthy"
    )
