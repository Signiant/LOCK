...
def rotate_auth0_key(config_map, username, **key_args):
    ...
    response = requests.get(
        key_args.get("url_api")
        + "emails/provider?fields=credentials%2Cname%2Cdefault_from_address%2Csettings%2Cenabled&include_fields=true",
        headers=auth,
    )
    data = response.content.decode("utf8")
    data = json.loads(data)
    ...
    if values.DryRun is True:
        ...
        response = requests.patch(
            key_args.get("url_api") + "emails/provider", headers=header_auth, data=data
        )
